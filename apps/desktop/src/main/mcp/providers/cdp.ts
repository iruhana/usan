...
const SERVER_MATCHERS = ['chrome-devtools', 'chrome_devtools', 'devtools', 'cdp']
...
function resolveServer() {
  ...
  if (configs.length === 0) {
    return {
      ...
      connected: false,
      toolCount: 0,
      reason: 'No chrome-devtools MCP server is configured.',
    }
  }
  ...
  return {
    ...
    reason: 'chrome-devtools MCP server is configured but not connected.',
  }
}

class ChromeDevToolsProvider implements AutomationProviderAdapter {
  readonly provider = 'chrome-devtools' as const
  ...
  async callTool(toolName: string, args: Record<string, unknown>, _context?: AutomationCallContext): Promise<unknown> {
    ...
    if (!resolved.connected || !resolved.serverId) {
      throw new Error(resolved.reason ?? 'chrome-devtools MCP server is not available.')
    }
...
export const chromeDevtoolsProvider = new ChromeDevToolsProvider()
